...
6. For DbIntelligence local setup, prefer PowerShell scripts under `src-templates/DbIntelligence/scripts/` (including user-scoped Node via `Initialize-DbIntelligenceNode.ps1` / fnm — no admin). Prefer Codegraph via `fnm exec --using=lts-latest -- npm i -g @colbymchenry/codegraph` when fnm is present.
