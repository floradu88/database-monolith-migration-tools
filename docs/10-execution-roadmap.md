...
- scan repositories with **DbIntelligence** (`src-templates/DbIntelligence/scripts/*.ps1` — see [`HOW-TO-USE.md`](../HOW-TO-USE.md)); use `Initialize-DbIntelligenceNode.ps1` for user-scoped Node/npm and Codegraph via `fnm exec` when fnm is present; use `Invoke-DbIntelligenceBatchIndex.ps1` for parent folders such as `D:\code\projects` or `C:\code`;
