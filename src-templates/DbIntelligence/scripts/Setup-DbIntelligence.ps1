...
if (-not $SkipPrereqs) {
    ...
    if ($Yes) {
        ...
        & (Join-Path $Scripts "Install-DbIntelligencePrereqs.ps1")
    }
...
Write-Host "Next:"
Write-Host "  .\scripts\Invoke-DbIntelligenceReady.ps1 `"D:\path\to\repo`"   # path-only one-shot"
...
Write-Host "Node tip: .\scripts\Initialize-DbIntelligenceNode.ps1 -Install -Yes  (fnm Node + Codegraph via fnm exec --using=lts-latest)"
